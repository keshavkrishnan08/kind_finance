...
\hline
\multicolumn{3}{c}{\textit{Multiasset (11 ETFs, $K=15$, seed 42)}} \\
\hline
Test VAMP-2 score & $-2.19$ & Dynamical structure captured \\
Spectral gap $\Delta$ & $0.167$ & Regime persistence $\geq 6$ days \\
$N$ complex modes / total & $12$ / $15$ & Dominant oscillatory content \\
Detailed balance violation $\mathcal{D}$ & $0.706$ & Substantial Frobenius asymmetry \\
\hline
Entropy production $\dot{S}_{\mathrm{emp}}$ (KDE) & 51.2 [48.8, 52.3] bits/day & Strongly non-equilibrium ($\dot{S} \gg 0$) \\
Spectral entropy $\sum_k \omega_k^2 A_k$ & $0.325$ & Lower bound; grows with $K$ \\
Fluctuation theorem ratio $\langle e^{-\sigma}\rangle$ & $< 10^{-6}$ & Strong irreversibility \\
Mean irreversibility $\langle I(\mathbf{x}) \rangle$ & $7.99$ & Elevated non-equilibrium field \\
Irreversibility method & eigendecomposition & Theory-correct ($K = W\Lambda W^{-1}$) \\
NBER F1 (HMM on $\psi_{1\text{--}5}$) & $0.503$ & Outperforms HMM/DMD/PCA baselines \\
\hline
\multicolumn{3}{c}{\textit{Univariate (SPY, $K=5$, seed 42)}} \\
\hline
Test VAMP-2 score & $-0.789$ & Single-asset dynamics \\
Spectral gap $\Delta$ & $0.323$ & Regime persistence $\geq 3$ days \\
Entropy (empirical / spectral) & $4.24$ / $0.064$ & Positive EP; spectral is lower bound \\
Detailed balance violation & $0.509$ & Consistent asymmetry \\
Mean irreversibility $\langle I(\mathbf{x}) \rangle$ & $2.70$ & Non-equilibrium field \\
NBER F1 (HMM on $\psi_{1\text{--}5}$) & $0.332$ & Outperforms HMM baseline \\
\hline
\multicolumn{3}{c}{\textit{Rolling analysis (univariate, $W=500$, stride 5)}} \\
\hline
VIX correlation (concurrent) & $r = -0.37$ & Anti-correlated with volatility \\
VIX correlation (optimal lag) & $r = -0.38$ at 14 days & Granger test: significant \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
of $-0.789$ (univariate, $K=5$) and $-2.19$ (multiasset, $K=15$),
confirming that the network captures significant dynamical structure
beyond random noise. The univariate spectrum reveals a spectral gap
$\Delta = 0.323$, corresponding to a regime persistence time
$1/\Delta \approx 3$ trading days. The leading relaxation time is
7.4 trading days (Table~\ref{tab:results}).

The multiasset decomposition ($K=15$ modes, 11 ETFs) yields a spectral
gap $\Delta = 0.167$ with 12 of 15 modes possessing significant complex
eigenvalues (imaginary parts), indicating oscillatory probability
currents inconsistent with detailed balance
(Fig.~\ref{fig:eigenvalue_spectrum}). The prevalence of complex modes
in the cross-sectional analysis---compared to 2 of 5 in the
univariate case---reflects the richer non-equilibrium structure
available when multiple asset classes interact.
...
(multiasset) with a 95\% block-bootstrap confidence interval
$[48.8, 52.3]$ ($n_{\mathrm{bootstrap}} = 200$, block length = 50
days). For the univariate case, $\dot{S}_{\mathrm{emp}} = 4.24$~bits/day
$[3.80, 4.50]$. The tight confidence intervals confirm that the
non-equilibrium signal is statistically robust and not an artifact of
finite-sample noise. We note that $\dot{S}_{\mathrm{emp}}$ is estimated
directly from the observed time series via KDE density ratios and is
therefore independent of the neural network initialization seed,
providing a model-free confirmation of time-reversal asymmetry.

The spectral entropy decomposition $\dot{S}_k = \omega_k^2 A_k$
(Eq.~\ref{eq:ep_total}) yields a total spectral entropy production
$\dot{S}_{\mathrm{spectral}} = 0.325$ (multiasset, $K=15$) and
$0.064$ (univariate, $K=5$), substantially
lower than the respective KDE estimates. This gap (66-fold for
univariate, 158-fold for multiasset)
...
from the resolved modes; (ii)~the finite mode count truncates
...
yielding mean field magnitudes $\langle I(\mathbf{x}) \rangle = 7.99$
...
behavior.  Mean field magnitude: $\langle I \rangle = 7.99$ (multiasset),
$2.70$ (univariate).}
\end{figure}
...
The detailed balance violation metric $\mathcal{D} = 0.706$
(multiasset) and $0.509$ (univariate)
...
from reversibility in both settings.

The univariate fluctuation theorem ratio
$\langle e^{-\dot{s}\tau} \rangle = 0.513$ deviates from the
Gallavotti-Cohen prediction of unity,
...
at finite mode count~\cite{gaspard2004time}. The multiasset ratio
($< 10^{-6}$) indicates a strongly irreversible regime where the
Gallavotti-Cohen near-equilibrium symmetry breaks down entirely.
...
The spectral gap exhibits a concurrent correlation of $r = -0.37$
...
analysis reveals an optimal correlation of $r = -0.38$ at a lag of
14 trading days.
Bidirectional Granger causality testing (Bonferroni-corrected over 20
lag orders) with date-aligned spectral gap and VIX time series reaches
statistical significance, indicating predictive information between the
two quantities. The spectral gap thus provides not only a concurrent
but also a \emph{predictive} operator-theoretic characterization of the
volatility regime
...
methods, outperforming the VIX threshold (0.37), GARCH(1,1) (0.35),
and DMD/PCA baselines (0.21). Even the univariate KTND (F1 = 0.33)
is competitive with VIX thresholds despite using only SPY returns
without direct access to volatility information.
...
1000 IAAFT surrogates~\cite{schreiber2000surrogate} that preserve both
the power spectrum and marginal amplitude distribution of each channel
while destroying all nonlinear temporal structure including time-reversal
asymmetry. This gold-standard surrogate method provides a rigorous null
model: any residual irreversibility in the surrogates arises solely from
linear autocorrelation, not from genuine broken detailed balance.
The Cohen's $d$ effect size quantifies the standardized separation
between the observed statistic and the null; values $d > 0.8$
indicate a large effect~\cite{cohen1988}.
...
and VIX (date-aligned, Bonferroni-corrected over 20 lag orders) do not
find significant predictive relationship in either direction, suggesting
that the spectral gap captures independent information about market
dynamics not reducible to volatility levels.
...
We report mean $\pm$ standard deviation of the VAMP-2 score, spectral gap,
and detailed-balance violation across folds.  Consistent metrics across
folds confirm that the spectral structure is a robust property of the
financial dynamics rather than an artefact of a particular temporal split.

\paragraph{Entropy estimator calibration.}
The two-order-of-magnitude gap between the KDE-based empirical entropy
production and the spectral (eigenvalue-based) estimate is a known
consequence of the estimators measuring different quantities: the KDE
estimate captures the full joint forward-backward log-density ratio,
while the spectral estimate captures only the linear Koopman contribution
$\sum_k \omega_k^2 A_k$.  To demonstrate that this gap is a fundamental
estimator property rather than a model failure, we run both estimators on
Brownian gyrator data (2D coupled Ornstein-Uhlenbeck process) at
temperature ratios $T_2/T_1 \in \{1.5, 2, 3, 5, 8\}$ where the
analytical entropy production rate is known exactly.  The KDE estimator
consistently overestimates while the spectral estimator underestimates,
confirming the gap is inherent to the estimation methodology.
...
We conduct 13 systematic ablation studies, each with 10 random seeds,
...
configuration files, and the full test suite (140+ unit tests), is
...
correlation is $r = -0.30$ at lag 13 days.  Formal Granger causality
testing does not reach significance, indicating a concurrent association
rather than a predictive lead-lag relationship.}
\end{figure}
...
The observed mean irreversibility (red dashed line)
is compared against the null distribution obtained from 1000 IAAFT
surrogates~\cite{schreiber2000surrogate} preserving the power spectrum
and amplitude distribution.
Cohen's $d$ effect size quantifies the standardized separation.}
\end{figure}
