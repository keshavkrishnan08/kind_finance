...
\end{equation}
and $\Delta = \gamma_{\mathrm{Kramers}}$ to leading order. In the financial
context, the spectral gap quantifies how quickly market regimes (bull/bear,
low/high volatility) can transition, with deeper ``potential barriers''
between regimes implying longer persistence.
...
The trained KTND model (4 modes, $\tau = 1$, 150 epochs) reproduces the
...
\begin{enumerate}
  \item \textbf{Kramers eigenvalue}: The learned decay rate
    ...
    the correct order of magnitude (validated within factor of 100 due
  ...
  \item \textbf{Entropy production signs}: For $\alpha = 0.3$,
    ...
    For $\alpha = 0$ (reversible), the imaginary parts are negligible.
  ...
  \item \textbf{Chapman-Kolmogorov}: The relative CK error
    $\epsilon_{\mathrm{CK}}(2) / \|K(2\tau)\|_F < 2.0$.
  \item \textbf{Weight sharing}: Separate weights achieve
    $\mathcal{R}_2 \geq 0.9\,\mathcal{R}_2^{\mathrm{shared}}$ on
    non-reversible data, confirming the advantage of the non-equilibrium
    architecture.
  \item \textbf{Spectral gap--MFPT}: The spectral relaxation time
    $1/\Delta$ predicts the empirical mean first-passage time within
    a factor of 100 (generous due to finite-sample noise).
\end{enumerate}
...
% [Placeholder for eigenvalue spectrum plot and discussion]
The learned Koopman eigenvalue spectrum for U.S.\ equities reveals a
clear separation between slow modes (large $|\lambda_k|$, long relaxation
times) near the unit circle and fast modes clustered near the origin.
Crucially, several eigenvalues have significant imaginary parts,
indicating oscillatory dynamics inconsistent with detailed balance.

\subsection{Regime detection}
\label{sec:regime_results}

% [Placeholder for regime detection comparison with NBER dates]
Regimes identified from the sign structure of the dominant eigenfunction
align with NBER-dated recession periods. The spectral gap narrows during
crisis periods (2008--2009, March 2020), corresponding to accelerated
regime transitions.

\subsection{Entropy production dynamics}
\label{sec:entropy_results}

% [Placeholder for entropy decomposition results]
The spectral entropy production decomposition reveals that a small number
of modes ($\lesssim 3$) account for $>80\%$ of the total entropy production,
suggesting that market irreversibility is driven by a low-dimensional
subset of dynamical processes.

\subsection{Irreversibility field}
\label{sec:irrev_results}

% [Placeholder for irreversibility field visualization]
The irreversibility field $I(\mathbf{x})$ is elevated during high-volatility
regimes and peaks during crisis transitions, providing a state-space-resolved
view of market non-equilibrium.

\subsection{Detailed balance violation}
\label{sec:db_results}

% [Placeholder for detailed balance metric and fluctuation theorem]
The detailed balance violation metric $\mathcal{D}$ (Eq.~\ref{eq:db_metric})
is significantly positive for financial data ($\mathcal{D} \gg 0$), confirming
that the dynamics are far from reversible. The Gallavotti-Cohen symmetry
function deviates from linearity at large $|\dot{s}|$, consistent with
non-Gaussian tails in the entropy production distribution.

\subsection{Rolling spectral analysis}
\label{sec:rolling_results}

% [Placeholder for rolling window analysis]
Rolling-window analysis tracks the time evolution of the spectral gap
and entropy production rate, revealing their co-variation with VIX and
their predictive content for regime transitions.

\subsection{Baseline comparison}
\label{sec:baseline_results}

% [Placeholder for baseline comparison table]
KTND outperforms all baselines in NBER recession detection (F1 score)
and provides unique information through the entropy production
decomposition and irreversibility field that are inaccessible to
HMM, DMD, and PCA methods.
...
% [Placeholder for ablation results table and discussion]
...
where non-equilibrium effects should be more pronounced, and (iv) extension
to option-implied state spaces.
