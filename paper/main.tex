...
\begin{enumerate}
  ...
  \item \textbf{Comprehensive validation.} We validate on non-reversible
    Langevin dynamics (recovering Kramers' rates), apply to 19 years of
    U.S.\ equity data with IAAFT surrogate testing ($p < 10^{-4}$,
    Cohen's $d = 33.0$), and benchmark against HMM, DMD, GARCH, and
    PCA baselines (Secs.~\ref{sec:synthetic}--\ref{sec:results}).
\end{enumerate}
...
Entropy (empirical / spectral) & $4.20\ [3.82,\, 4.58]$ / $0.064$ & Positive EP; spectral is lower bound \\
...
The spectral decomposition $\dot{S}_k = \omega_k^2 A_k$
(Eq.~\ref{eq:ep_total}) yields
$\dot{S}_{\mathrm{spectral}} = 1.53$ (multiasset, $K=15$) and
$0.064$ (univariate, $K=5$), lower than the KDE estimates by factors
of $\sim$33 and $\sim$66, respectively.  This gap is expected: the
perturbative decomposition (Eq.~\ref{eq:ep_mode}) resolves only the
leading-order contribution from the $K$ retained modes, while the
KDE estimator captures the \emph{full} Kullback-Leibler divergence
between forward and time-reversed path densities across all degrees of
freedom.  The spectral estimate therefore serves as a \emph{lower bound}
that converges from below as $K$ increases, while providing a
physically interpretable decomposition into mode contributions.
...
indicating that market irreversibility is concentrated in a
