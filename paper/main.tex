...
that anticipates NBER-dated recessions. Comparisons with hidden Markov
models, dynamic mode decomposition, and PCA baselines---together with
...
observables for U.S.\ equity markets (SPY, 1994--2025). Uncertainties
are 95\% block-bootstrap confidence intervals where available.}
\begin{ruledtabular}
\begin{tabular}{lll}
Observable & Value & Interpretation \\
\hline
Test VAMP-2 score (univariate) & 2.70 & Dynamical structure captured \\
Test VAMP-2 score (multiasset) & 1.44 & 11-ETF cross-asset dynamics \\
Spectral gap $\Delta$ & 0.040 & Regime persistence $\geq 25$ days \\
$N$ complex modes / total & 8 / 10 & Dominant oscillatory (non-equilibrium) content \\
Complex fraction & 0.54 & Oscillatory $>$ relaxational content \\
Singular value range & [0.14, 0.90] & Well-resolved mode hierarchy \\
Eigenvalue magnitude range & [0.45, 0.70] & Relaxation times 6--14 days \\
\hline
Entropy production $\dot{S}_{\mathrm{emp}}$ (KDE) & 51.2 [48.9, 52.3] bits/day & Strongly non-equilibrium ($\dot{S} \gg 0$) \\
Spectral entropy $\sum_k \omega_k^2 A_k$ & 1.53 & Lower bound; grows with $K$ \\
Detailed balance violation $\mathcal{D}$ & 0.73 & Substantial Frobenius asymmetry \\
Fluctuation theorem ratio & 0.87 & Approximate GC compliance \\
...
VIX correlation (optimal lag) & $r = -0.29$ at $-8$ days & Suggestive lead (needs Granger test) \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
Cross-correlation analysis suggests the spectral gap may lead VIX by
approximately 8 trading days ($r = -0.29$), though the improvement over
the concurrent correlation ($r = -0.27$) is modest.}
\end{figure}
...
$\dot{S}_{\mathrm{spectral}} = 1.53$, substantially lower than the
KDE estimate $\dot{S}_{\mathrm{emp}} = 51.2$.  This $\sim$33-fold gap
...
(3 modes: 0.58; 5 modes: 0.93; 10 modes: 1.57; 20 modes: 3.20;
50 modes: 6.91), supporting the interpretation of the spectral estimate
...
The spectral gap exhibits a concurrent correlation of $r = -0.27$
with the VIX volatility index ($n = 1563$ windows). Cross-correlation
analysis reveals that the optimal correlation occurs at a lag of
$-8$ trading days ($r = -0.29$), suggesting that spectral gap changes
may \emph{lead} VIX movements by approximately 8 days. However, the
correlation improvement over the concurrent value is modest
($\Delta r = 0.02$) and formal Granger causality testing would be
required to establish a statistically significant lead-lag
relationship. We note this as a suggestive but preliminary finding
that spectral properties---which encode the operator-level dynamics---may
shift before realized volatility responds
...
KTND's own regime assignments, derived from the sign structure of the
dominant eigenfunction $\psi_1(\mathbf{x})$, are included in
Table~\ref{tab:baselines}. The VIX threshold baseline achieves the
...
recession dating. The HMM baseline achieves high accuracy (0.88) but
extremely low precision (0.04) and recall (0.02), indicating that its
regime assignments are poorly aligned with NBER recession labels despite
capturing some latent structure. DMD and PCA produce identical regime
assignments with moderate F1 (0.21).

Beyond regime detection accuracy---where KTND is competitive but not
dominant---the framework provides unique physical observables: the entropy
production decomposition, irreversibility field, and fluctuation theorem
diagnostics are fundamentally inaccessible to the baseline methods.
The crisis anticipation lead times (GFC: 31 days for HMM, 1--2 days for
DMD/VIX; COVID: 20 days for HMM, 107 days for VIX) vary substantially
across methods, suggesting that different models capture different
aspects of pre-crisis dynamics.
...
NBER-dated recession periods. Lead times indicate how many trading days
before the official recession start the method first signals a regime
change.}
\begin{ruledtabular}
\begin{tabular}{lddddd}
Method & \multicolumn{1}{c}{Acc.} & \multicolumn{1}{c}{Prec.} &
  \multicolumn{1}{c}{Rec.} & \multicolumn{1}{c}{F1} &
  \multicolumn{1}{c}{GFC lead} \\
\hline
KTND ($\psi_1$ sign) & \multicolumn{5}{c}{\textit{(values from pipeline run)}} \\
HMM ($K\!=\!3$) & 0.88 & 0.04 & 0.02 & 0.03 & 31 \\
DMD ($K\!=\!10$) & 0.83 & 0.16 & 0.28 & 0.21 & 1 \\
PCA + $K$-means & 0.83 & 0.16 & 0.28 & 0.21 & 1 \\
VIX threshold & 0.90 & 0.35 & 0.38 & 0.37 & 2 \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
against NBER-dated recession periods. The VIX threshold baseline
achieves the highest F1 due to the strong contemporaneous VIX--recession
relationship. KTND provides complementary non-equilibrium diagnostics
...
(default), to isolate the contribution of each model component.
Preliminary results reported here use 3 seeds; full 10-seed runs are
in progress:
...
over 3 seeds, preliminary; 10-seed runs pending).  VAMP-2 is the
negative VAMP-2 loss (more negative = higher score).  Variants marked
$\dagger$ produced results indistinguishable from the baseline due to a
configuration override bug in the ablation runner (now fixed;
re-running pending).  The rolling window sweep was removed as it only
affects post-hoc analysis, not model training.}
\begin{ruledtabular}
...
\begin{enumerate}
  ...
  \item \textbf{Causal interpretation}: While the spectral decomposition
    ...
    irreversibility.
  \item \textbf{VIX lead-lag relationship}: The observed cross-correlation
    optimum at $-8$ days ($r = -0.29$ vs.\ $r = -0.27$ concurrent) is
    suggestive but has not been subjected to formal Granger causality
    testing. The correlation improvement is modest ($\Delta r = 0.02$),
    and robust statistical inference requires additional validation.
  \item \textbf{Ablation seed count}: Preliminary ablation results use
    3 seeds per variant (sufficient for trend identification). Final
    results with 10 seeds are needed for reliable standard error
    estimates.
