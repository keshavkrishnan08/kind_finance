...
$\dot{S}_k = \omega_k^2 A_k$, and a pointwise irreversibility field
...
\begin{enumerate}
  \item \textbf{Non-equilibrium Koopman framework.}  A dual-lobe neural
    ...
    $\dot{S}_k \propto \omega_k^2 A_k$ from the Kullback-Leibler divergence
...
\end{equation}
This quadratic scaling is the fundamental reason that per-mode
contributions scale as $\omega_k^2$: for each complex eigenvalue
$\lambda_k = |\lambda_k|\,e^{i\omega_k\tau}$, the eigenvector
asymmetry $u_k \neq v_k$ is induced by the oscillatory component
$\omega_k$, and the KL divergence inherits the $\omega_k^2$
dependence through~\eqref{eq:ep_secondorder}.

To obtain a mode-resolved expression, we work in the
finite-dimensional Koopman matrix representation.
...
The perturbative entropy production rate to leading order in
$\mathbf{K}_A$ is~\cite{gaspard2004time,maes2003time}
...
\end{equation}
In the eigenbasis of $\mathbf{K}_S$ (with eigenvalues $\mu_k$ and
decay rates $\gamma_k = -\ln\mu_k/\tau$), the trace evaluates to
\begin{equation}\label{eq:ep_modesum}
  \ep = \frac{2}{\tau}\sum_{k=1}^{K}
    \frac{(\mathbf{K}_A)_{kk}^2}{\mu_k}
    + \text{off-diag.\ terms},
\end{equation}
where the diagonal contributions $(\mathbf{K}_A)_{kk}$ are related
to the oscillation frequencies $\omega_k$ through
$\mathrm{Im}(\lambda_k) \approx (\mathbf{K}_A)_{kk}$ at leading
order in perturbation theory.

For the spectral irreversibility index reported in this work, we
adopt the simplified form
\begin{equation}\label{eq:ep_mode}
  \dot{S}_k = \omega_k^2\, A_k,
\end{equation}
where $\omega_k = \arg(\lambda_k)/\tau$ and
$A_k = \langle |\psi_k|^2 \rangle_\mu$ is the eigenfunction amplitude.
The total resolved spectral irreversibility is
\begin{equation}\label{eq:ep_total}
  \ep_{\mathrm{spectral}} = \sum_{k=1}^{K} \omega_k^2\, A_k.
\end{equation}
This captures two essential physical features:
(i)~modes with $\omega_k = 0$ (real eigenvalues) contribute zero,
consistent with purely relaxational dynamics satisfying detailed
balance; and (ii)~the contribution scales quadratically in
frequency, so faster oscillatory modes---corresponding to
stronger probability currents---dominate the irreversibility budget.

We stress three limitations.  First,
Eq.~\eqref{eq:ep_mode} omits the decay-rate prefactors
$\sim 1/\gamma_k$ present in the full perturbative
result~\eqref{eq:ep_perturbative}; the simplified form preserves
the relative ranking of mode contributions (which modes dominate
irreversibility) but not the absolute magnitude.
Second, the sum runs over only the $K$ resolved Koopman modes;
unresolved higher-frequency modes ($k > K$) contribute additional
irreversibility that the spectral index does not capture.  Because
$\dot{S}_k \propto \omega_k^2$, these fast modes can carry a
substantial fraction of the total entropy production.
Third, the perturbative expansion~\eqref{eq:ep_perturbative} is valid
when $\|\mathbf{K}_A\|_F \ll \|\mathbf{K}_S\|_F$; for strongly
non-equilibrium dynamics, higher-order terms become relevant.
We verify the self-consistency empirically through the
entropy production consistency loss (Sec.~\ref{sec:loss}).
...
Spectral index $\sum_k \omega_k^2 A_k$ & $1.53$ & Resolved-mode irreversibility \\
...
Entropy (KDE / spectral index) & $4.20\ [3.82,\, 4.58]$ / $0.064$ & Positive EP; index captures $K$ modes \\
...
The spectral irreversibility index $\sum_k \omega_k^2 A_k$
(Eq.~\ref{eq:ep_total}) yields
$\dot{S}_{\mathrm{spectral}} = 1.53$ (multiasset, $K=15$) and
$0.064$ (univariate, $K=5$).  These values are smaller than the
KDE estimates by factors of $\sim$33 and $\sim$66, respectively.
The gap has three well-understood sources:
(i)~the spectral index sums over only the $K$ resolved Koopman modes;
because $\dot{S}_k \propto \omega_k^2$, the unresolved high-frequency
modes ($k > K$)---which by construction have the largest $\omega_k$
values---carry a substantial and potentially dominant fraction of the
total irreversibility;
(ii)~the simplified index~\eqref{eq:ep_mode} omits the decay-rate
prefactors $\sim 1/\gamma_k$ present in the full perturbative
result~\eqref{eq:ep_perturbative}, which amplify contributions from
slowly decaying modes; and
(iii)~the perturbative expansion itself underestimates entropy
production in the strongly non-equilibrium regime where
$\|\mathbf{K}_A\|_F$ is not small relative to $\|\mathbf{K}_S\|_F$.
The spectral index is therefore \emph{not} an estimate of the full
entropy production; rather, it provides a mode-resolved decomposition
of the \emph{resolved} irreversibility, identifying which dynamical
timescales contribute most.
...
The entropy production decomposition $\dot{S}_k = \omega_k^2 A_k$
...
\begin{enumerate}
  ...
  \item \textbf{Perturbative entropy decomposition.}  The per-mode
    $\dot{S}_k = \omega_k^2 A_k$ is valid in the weakly dissipative
...
per-mode contributions $\dot{S}_k = \omega_k^2 A_k$ with a pointwise
